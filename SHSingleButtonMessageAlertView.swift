...
open class SHSingleButtonMessageAlertView: SHUIView {
    ...
    private func setUI() {
        ...
        let confirmButton: SHUIButton = self.confirmButton
        ...
        confirmButton.saiha_addSeparator(color: self.separatorColor, position: .top, leftEdge: self.separatorEdge, rightEdge: self.separatorEdge)
        let line: SHUIView = SHUIView()
        line.backgroundColor = self.separatorColor
        self.mainContentView.addSubview(line)
        line.snp.makeConstraints { make in
            make.left.equalTo(confirmButton.snp.right)
            make.width.equalTo(1)
            make.top.equalTo(confirmButton.snp.top).offset(CGFloat.saiha_verticalSize(num: 14))
            make.bottom.equalTo(confirmButton.snp.bottom).offset(CGFloat.saiha_verticalSize(num: -14))
        }
    }
    
    func addView(title: String?, customView: UIView, inViewController: Bool = false, confirmAction: @escaping (() -> Void)) {
        if inViewController {
    ...
    public static func show(title: String?, message: String, inViewController: Bool = false, viewConfiguration: ((_ singleButtonMessageAlertView: SHSingleButtonMessageAlertView) -> Void)?, confirmAction: @escaping (() -> Void)) {
        let singleButtonMessageAlertView: SHSingleButtonMessageAlertView = SHSingleButtonMessageAlertView()
        singleButtonMessageAlertView.label.text = message
        viewConfiguration?(singleButtonMessageAlertView)
        singleButtonMessageAlertView.addView(title: title, customView: singleButtonMessageAlertView.contentView, inViewController: inViewController, confirmAction: confirmAction)
    }
}
