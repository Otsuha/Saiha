...
public class SHCalendarView: SHUIView {
    
    // MARK: - Property.
    
    private var controlView: SHUIView = {
        let view: SHUIView = SHUIView()
        return view
    }()
    private var dateDisplayLabel: SHUILabel = {
        let label: SHUILabel = SHUILabel()
        label.font = .systemFont(ofSize: 16, weight: .medium)
        label.textAlignment = .center
        return label
    }()
    private var leftArrow: SHUIButton = {
        let button: SHUIButton = SHUIButton()
        let bundle: Bundle = Bundle.saihaBundle()
        let leftArrowImage: UIImage? = UIImage.saiha_image(name: "arrow_left", in: bundle)
        button.setImage(leftArrowImage, for: .normal)
        button.contentMode = .scaleAspectFit
        return button
    }()
    private var rightArrow: SHUIButton = {
        let button: SHUIButton = SHUIButton()
        let bundle: Bundle = Bundle.saihaBundle()
        let rightArrowImage: UIImage? = UIImage.saiha_image(name: "arrow_right", in: bundle)
    ...
    private var cancelButton: SHUIButton = {
        ...
        button.setTitleColor(UIColor.saiha_colorWithHexString("#3951C4"), for: .normal)
        button.layer.cornerRadius = 22.5
        button.layer.borderColor = UIColor.saiha_colorWithHexString("#3951C4").cgColor
        button.layer.borderWidth = 1
        return button
    }()
    private var confirmButton: SHUIButton = {
        let button: SHUIButton = SHUIButton()
        button.backgroundColor = UIColor.saiha_colorWithHexString("#3951C4")
    ...
    public weak var delegate: SHCalendarViewDelegate?
    ...
    private var currentDate: SHDate = SHDate()
    ...
    // 根据当前日期推算几个月之前的日期。
    private func calculationDate(value: Int) -> SHDate {
        let date: SHDate = SHCalendar.current.date(
            byAdding: .month,
            value: value,
            to: SHDate()
        )
    ...
    @objc private func leftArrowHandle(sender: SHUIButton) {
        ...
        self.currentIndex -= 1
        self.currentDate = SHCalendar.current.date(
            byAdding: .month,
            value: -1,
            to: self.currentDate
        )
    ...
    @objc private func rightArrowHandle(sender: SHUIButton) {
    ...
    private func getCurrentMonthCell() -> SHCalendarDateCell? {
        let cells: [SHCalendarDateCell] = self
            .mainCollectionView
            .visibleCells as! [SHCalendarDateCell]
        for cell in cells {
    ...
    //    public func jumpToDate(date: SHDate) {
    ...
    public func refresh() {
        DispatchQueue.main.async { [weak self] in
            ...
            let currentCell: SHCalendarDateCell? = strongSelf
                .mainCollectionView
                .cellForItem(at: currentIndexPath) as? SHCalendarDateCell
            ...
            let prefixCell: SHCalendarDateCell? = strongSelf
                .mainCollectionView
                .cellForItem(at: prefixIndexPath) as? SHCalendarDateCell
            let afterIndexPath: IndexPath = IndexPath(row: strongSelf.currentIndex + 1, section: 0)
            let afterCell: SHCalendarDateCell? = strongSelf
                .mainCollectionView
                .cellForItem(at: afterIndexPath) as? SHCalendarDateCell
...
extension SHCalendarView: UICollectionViewDelegate {
    
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        let cell: SHCalendarDateCell? = collectionView
            .cellForItem(at: indexPath) as? SHCalendarDateCell
        if cell != nil {
...
extension SHCalendarView: UIScrollViewDelegate {
    
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        self.dateDisplayLabel.text = String(self.currentMonth().toString().prefix(7))
        saiha_print(String(self.currentMonth().toString().prefix(7)))
    }
}

// MARK: - UICollectionViewDataSource.

extension SHCalendarView: UICollectionViewDataSource {
    ...
    private func judgeRange(selectedBeginDateString: String) -> (start: String, end: String) {
        let selectedBeginDate: SHDate = SHDate(date: self.beginDate!)
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var cell: SHCalendarDateCell = SHCalendarDateCell()
        if self.allowManualScroll {
            cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: "DateCell",
                for: indexPath
            ) as! SHCalendarDateCell
            let date: SHDate = self.calculationDate(value: self.dateArray[indexPath.row])
            cell.date = date
        ...
        if let delegate = self.delegate {
            cell.viewForCellHandle = { (view, size, date) in
                let view: SHUIView? = delegate.calendarView?(
                    calendarView: view,
                    cellItemSize: size,
                    viewForCellAt: date
                )
