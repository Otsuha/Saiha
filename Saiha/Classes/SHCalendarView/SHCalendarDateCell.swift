...
class SHCalendarDateCell: UICollectionViewCell {
    
    var date: SHDate!
    var calendarView: SHCalendarView!
    ...
    var handleTapDate: ((_ view: SHCalendarView, _ cell: SHCalendarDateButton) -> Void)?
    var viewForCellHandle: ((_ view: SHCalendarView, _ cellItemSize: CGSize, _ date: SHDate?) -> SHUIView?)?
    var enableSelectDateHandler: ((_ view: SHCalendarView, _ date: SHDate, _ cell: SHCalendarDateButton) -> Bool)?
    
    //var enableDateRange: (start: String, end: String)? = nil
    
    private var currentDateArray: [SHDate?] = [SHDate?](repeating: SHDate(), count: 49)
            
    private var dateView: UIView!
    
    private var prefixSelectedButton: SHUIButton?
    ...
    private func update() {
        let calendar: SHCalendar = SHCalendar.current
        let firstDateOfMonth: SHDate = calendar.getFirstDateOfMonth(date: self.date)
        
        // - 如果需要填充上个月和下个月的日期，需要提前计算好上个月的最后一天以避免在下面的循环中重复计算。
        
        var lastDateOfLastMonth: SHDate = SHDate()  // 上个月的最后一天。
        if self.fillWithLastAndNextMonthDay {
            let day: SHDate = calendar.date(byAdding: .day, value: -1, to: firstDateOfMonth)
    ...
    func refreshDateView(beginTime: String?, endTime: String?) {
        ...
        for i in 0...48 {
            let dateButton: SHCalendarDateButton = SHCalendarDateButton()
...
//                )
    ...
    func beginRefreshRangeButtonUI(
        ...
        for i in 7...48 {
            let button: SHCalendarDateButton = self
                .dateView
                .viewWithTag(100 + i) as! SHCalendarDateButton
    ...
    private func rangeButtonUI(
        beginTime: String?,
        endTime: String?,
        dateButton: SHCalendarDateButton
    ) {
        // 开始和结束日期都没选，保持原样。
...
//                dateButton.backgroundColor = UIColor.saiha_colorWithHexString("#3951C4")
...
//                dateButton.backgroundColor = UIColor.saiha_colorWithHexString(
//                    "#3951C4",
//                    alpha: 0.2
//                )
//                dateButton.setTitleColor(.white, for: .normal)
...
// MARK: - SHCalendarDateCell.

extension SHCalendarDateCell {
    
    @objc func handleTapDateButton(sender: SHCalendarDateButton) {
        sender.isSelected = !sender.isSelected
        self.handleTapDate?(self.calendarView, sender)
    }
}

// MARK: - SHCalendarDateButton.

public class SHCalendarDateButton: SHUIButton {
    ...
    public var date: SHDate?
    
    var selectStatus: SHCalendarDateButton.SelectStatus = .range {
        willSet(newValue) {
            switch newValue {
            case .none:
            ...
            case .left:
                self.leftView.backgroundColor = .white
                self.rightView.backgroundColor = UIColor.saiha_colorWithHexString(
                    "#3951C4",
                    alpha: 0.2
                )
                self.mainButton.backgroundColor = UIColor.saiha_colorWithHexString("#3951C4")
                self.mainButton.setTitleColor(.white, for: .normal)
            case .right:
                self.leftView.backgroundColor = UIColor.saiha_colorWithHexString(
                    "#3951C4",
                    alpha: 0.2
                )
                self.rightView.backgroundColor = .white
            ...
            case .range:
                ...
                self.mainButton.backgroundColor = UIColor.saiha_colorWithHexString(
                    "#3951C4",
                    alpha: 0.2
                )
                self.mainButton.setTitleColor(.black, for: .normal)
    ...
    private var mainButton: SHUIButton = {
    ...
    private var leftView: SHUIView = {
        let view: SHUIView = SHUIView()
        view.backgroundColor = .white
        return view
    }()
    
    private var rightView: SHUIView = {
        let view: SHUIView = SHUIView()
        view.backgroundColor = .white
        return view
    }()
    ...
    fileprivate func customView(view: SHUIView?) {
        if view == nil {
            return
        }
        self.setTitle(nil, for: .normal)
        let backgroundView: SHUIView = SHUIView()
