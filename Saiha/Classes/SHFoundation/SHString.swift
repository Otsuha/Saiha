...
extension String {
    ...
    public func saiha_subString(start: Int, to: Int) -> String {
        let st: String.Index = self.index(startIndex, offsetBy: start)
        let en: String.Index = self.index(st, offsetBy: to)
        return String(self[st..<en])
    }
    
    public func saiha_subString(start: Int, length: Int) -> String {
        var len: Int = length
        if len == -1 {
            len = count - start
        }
    ...
    public func saiha_replace(from: Int, to: Int, withString: String) -> String {
    ...
    public func saiha_replace(from: Int, length: Int, withString: String) -> String {
        var text = self
    ...
    public func saiha_replace(string: String, withString: String) -> String {
...
extension String {
    ...
    public func saiha_isIntNumber() -> Bool {
        let scan: Scanner = Scanner(string: self)
        var val: Int = 0
        return scan.scanInt(&val) && scan.isAtEnd
    }

    public func saiha_isFloatNumber() -> Bool {
        let scan: Scanner = Scanner(string: self)
        var val: Float = 0
        return scan.scanFloat(&val) && scan.isAtEnd
    }

    public func saiha_isEmail() -> Bool {
        if self.count <= 0 {
            return false
        }
    ...
    private func getStringByRangeIntValue(_ string: String, location: Int, length: Int) -> Int {
        let c = string.saiha_subString(start: location, length: length)
        return Int(c)!
    }

    public func saiha_isIDNumber() -> Bool {
        ...
        // 检测省份身份行政区代码。
        let valueStart2: String = value.saiha_subString(start: 0, length: 2)
        // 标识省份代码是否正确。
        ...
        case 15:
            ...
            let yearStr: String = valueNSStr.saiha_subString(start: 6, length: 2)
        ...
        case 18:
            let valueNSStr: String = value
            let yearStr: String = valueNSStr.saiha_subString(start: 6, length: 4)
            year = Int(yearStr)!
            if year % 4 == 0 || (year % 100 == 0 && year % 4 == 0) {
                // 测试出生日期的合法性。
            ...
            if numberofMatch! > 0 {
                ...
                M = JYM.saiha_subString(start: Y, length: 1)
                let lastStr: String = valueNSStr.saiha_subString(start: 17, length: 1)
